...
impl MiniUart {
    ...
    pub fn new() -> MiniUart {
        ...
        registers.BAUD.write(270); // set baud to 115200 (divider 270)
        registers.LCR.or_mask(0b11); // 8 - bit mode
        registers.CNTL.write(0); // enable transmitter & receiver
    ...
    pub fn write_byte(&mut self, byte: u8) {
        while (self.registers.LSR.read() & (TxAvailable as u8)) == 0 {}
    ...
    pub fn has_byte(&self) -> bool {
        (self.registers.LSR.read() & (DataReady as u8)) != 0
    }
    ...
    pub fn wait_for_byte(&self) -> Result<(), ()> {
        match self.timeout {
            Some(t) => {
                ...
                while timer::current_time() - start_time < t {
                    if self.has_byte() {
                        return Ok(());
                    }
                }
...
mod uart_io {
    ...
    impl io::Read for MiniUart {
        fn read(&mut self, buf: &mut[u8]) -> Result<usize, io::Error> {
            let is_ready = self.wait_for_byte();
            match is_ready {
                Ok(_) => {
                    let mut idx = 0;
                    while idx < buf.len() {
                ...
                Err(_) => {
                    return Err(io::Error::new(io::ErrorKind::TimedOut, "MiniUart timeout"));
                }
            }
        }
    }
