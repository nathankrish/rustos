...
impl<'a, T: 'a> StackVec<'a, T> {
    ...
    pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
        if len > storage.len() {
            panic!("StackVec Error: len {} > storage len {}", len, storage.len());
        }
        StackVec {
            storage: storage,
            len: len
...
impl <'a, T> IntoIterator for StackVec<'a, T> {
    ...
    // can use into_slice because self is the StackVec itself
    fn into_iter(self) -> Self::IntoIter {
        return self.into_slice().iter();
    }
}
...
impl <'a, T> IntoIterator for &'a StackVec<'a, T> {
    ...
    // must use as_slice because self is a ref to a StackVec
    fn into_iter(self) -> Self::IntoIter {
